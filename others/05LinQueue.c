...
int fnDeQueue(int[], int*, int*);file:///media/hdk/2014-15/ODD/DS/ds-programs%20practice/05LinQueue.c
...
void fnEnQueueFront(int q[],int *f,int *r,int val)
{
    if(*f==-1)
    {
        *f=*r=0;
...
int fnDeQueueRear(int q[],int *f,int *r)
{
    int val=q[*r];
    if(*f==*r)
            *f=*r=-1;
